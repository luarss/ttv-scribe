...
from src.state import VodStatus, VodRecord, StreamerRecord, Platform
...
# Helper to build a Kick VOD API response dict in the real format
def _kick_vod(uuid, session_title, created_at, duration_ms, numeric_id=1):
    return {
        "id": numeric_id,
...
class TestCheckKickVods:
    """Tests for _check_kick_vods function"""

    def test_creates_vod_records(self, mock_state_manager):
        """Test that VodRecords are created for Kick VODs"""
        with patch("src.monitor.get_state_manager", return_value=mock_state_manager):
            with patch("src.monitor.KickClient") as mock_client_class:
                mock_client = MagicMock()
                mock_client.__enter__ = MagicMock(return_value=mock_client)
                mock_client.__exit__ = MagicMock(return_value=False)
                mock_client_class.return_value = mock_client

                mock_client.get_vods_by_slug.return_value = [
                    _kick_vod("abc-123", "Kick Stream 1", "2024-01-01 00:00:00", 5400000),
                    _kick_vod("xyz-456", "Kick Stream 2", "2024-01-02 00:00:00", 3600000, numeric_id=2),
                ]

                count = _check_kick_vods(
                    "testuser", "testuser",
                    max_duration_minutes=None, min_days_old=0,
                )

                assert count == 2
                vod1 = mock_state_manager.get_vod("abc-123")
                assert vod1 is not None
                assert vod1.streamer == "testuser"
                assert vod1.platform == Platform.KICK.value
                assert vod1.duration == 5400  # 5400000ms / 1000
                assert vod1.title == "Kick Stream 1"
                vod2 = mock_state_manager.get_vod("xyz-456")
                assert vod2 is not None
                assert vod2.duration == 3600

    def test_filters_by_max_duration(self, mock_state_manager):
        """Test that Kick VODs longer than max_duration are skipped"""
        with patch("src.monitor.get_state_manager", return_value=mock_state_manager):
            with patch("src.monitor.KickClient") as mock_client_class:
                mock_client = MagicMock()
                mock_client.__enter__ = MagicMock(return_value=mock_client)
                ...
                mock_client.get_vods_by_slug.return_value = [
                    _kick_vod("short", "Short", "2024-01-01 00:00:00", 1800000),
                    _kick_vod("long", "Long", "2024-01-01 00:00:00", 7200000, numeric_id=2),
                ]
    ...
    def test_filters_by_min_days_old(self, mock_state_manager):
        """Test that recent Kick VODs are skipped"""
        recent_time = datetime.now(timezone.utc) - timedelta(days=1)
        old_time = datetime.now(timezone.utc) - timedelta(days=5)
        ...
        with patch("src.monitor.get_state_manager", return_value=mock_state_manager):
            with patch("src.monitor.KickClient") as mock_client_class:
                ...
                mock_client.get_vods_by_slug.return_value = [
                    _kick_vod("recent", "Recent", recent_time.strftime("%Y-%m-%d %H:%M:%S"), 1800000),
                    _kick_vod("old", "Old", old_time.strftime("%Y-%m-%d %H:%M:%S"), 1800000, numeric_id=2),
                ]
    ...
    def test_skips_existing_vods(self, mock_state_manager):
        """Test that already-tracked Kick VODs are not re-added"""
        mock_state_manager.add_vod(VodRecord(vod_id="existing-id", streamer="testuser"))
        ...
        with patch("src.monitor.get_state_manager", return_value=mock_state_manager):
            with patch("src.monitor.KickClient") as mock_client_class:
                ...
                mock_client.get_vods_by_slug.return_value = [
                    _kick_vod("existing-id", "Existing", "2024-01-01 00:00:00", 1800000),
                    _kick_vod("new-id", "New", "2024-01-01 00:00:00", 1800000, numeric_id=2),
                ]
                ...
                assert count == 1
                assert mock_state_manager.get_vod("new-id") is not None

    def test_lookup_by_username_when_no_slug(self, mock_state_manager):
        """Test that channel is looked up by username when kick_slug is None"""
        with patch("src.monitor.get_state_manager", return_value=mock_state_manager):
            with patch("src.monitor.update_streamer") as mock_update:
                with patch("src.monitor.KickClient") as mock_client_class:
                    mock_client = MagicMock()
                    mock_client.__enter__ = MagicMock(return_value=mock_client)
                    mock_client.__exit__ = MagicMock(return_value=False)
                    ...
                    mock_client.get_channel_by_slug.return_value = {
                        "id": 123, "slug": "TestUser",
                    }
                    mock_client.get_vods_by_slug.return_value = []
                    ...
                    assert count == 0
                    mock_client.get_channel_by_slug.assert_called_once_with("testuser")
                    mock_update.assert_called_once_with("testuser", kick_slug="TestUser")

    def test_uses_existing_slug(self, mock_state_manager):
        """Test that existing kick_slug is used directly"""
        with patch("src.monitor.get_state_manager", return_value=mock_state_manager):
            with patch("src.monitor.KickClient") as mock_client_class:
                mock_client = MagicMock()
                mock_client.__enter__ = MagicMock(return_value=mock_client)
                ...
                _check_kick_vods(
                    "testuser", "my-kick-slug",
                    max_duration_minutes=None, min_days_old=0,
                )

                mock_client.get_channel_by_slug.assert_not_called()
                mock_client.get_vods_by_slug.assert_called_once_with("my-kick-slug")

    def test_streamer_not_found(self, mock_state_manager):
        """Test that count is 0 when channel is not found"""
        with patch("src.monitor.get_state_manager", return_value=mock_state_manager):
            with patch("src.monitor.KickClient") as mock_client_class:
                mock_client = MagicMock()
                mock_client.__enter__ = MagicMock(return_value=mock_client)
                ...
                mock_client.get_channel_by_slug.return_value = None
    ...
    def test_no_vods_returned(self, mock_state_manager):
        """Test that count is 0 when no VODs are returned"""
        with patch("src.monitor.get_state_manager", return_value=mock_state_manager):
            with patch("src.monitor.KickClient") as mock_client_class:
                mock_client = MagicMock()
                mock_client.__enter__ = MagicMock(return_value=mock_client)
    ...
    def test_skips_vod_with_no_uuid(self, mock_state_manager):
        """Test that VODs without video.uuid are skipped"""
        with patch("src.monitor.get_state_manager", return_value=mock_state_manager):
            with patch("src.monitor.KickClient") as mock_client_class:
                mock_client = MagicMock()
                mock_client.__enter__ = MagicMock(return_value=mock_client)
                ...
                mock_client.get_vods_by_slug.return_value = [
                    {"id": 1, "session_title": "No UUID", "created_at": "2024-01-01 00:00:00", "duration": 1800000},
                ]
    ...
    def test_dispatched_from_check_streamer_vods(self, mock_state_manager):
        """Test that _check_streamer_vods dispatches to _check_kick_vods for kick platform"""
        with patch("src.monitor.get_state_manager", return_value=mock_state_manager):
            with patch("src.monitor.KickClient") as mock_client_class:
                mock_client = MagicMock()
                mock_client.__enter__ = MagicMock(return_value=mock_client)
                ...
                mock_client.get_vods_by_slug.return_value = [
                    _kick_vod("dispatched-vod", "Dispatched", "2024-01-01 00:00:00", 3600000),
                ]

                count = _check_streamer_vods(
                    "testuser", "kick", None, None, None, "testuser",
                    max_duration_minutes=None, min_days_old=0,
                )

                assert count == 1
                vod = mock_state_manager.get_vod("dispatched-vod")
                assert vod is not None
                assert vod.platform == Platform.KICK.value
